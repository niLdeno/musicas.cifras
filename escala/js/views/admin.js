...
async function secaoEscala(alvo, appState) {
  ...
  if (!escala) {
    card.appendChild(el('p', { class: 'sub', text: 'Ainda não há escala para este mês.' }));
    const btn = el('button', { class: 'btn', style: 'margin-top:10px', html: '<i class="fa-solid fa-plus"></i> Criar escala deste mês' });
  ...
  card.appendChild(el('p', { class: 'sub', html: `Status: <b style="color:${escala.publicada ? 'var(--st-green)' : 'var(--st-orange)'}">${escala.publicada ? 'Publicada' : 'Rascunho'}</b> · versão ${escala.versao} · ${preenchidos} itens preenchidos` }));
  ...
  const pub = el('button', { class: `btn ${escala.publicada ? 'btn-ghost' : 'btn-gold'}`,
    html: escala.publicada ? '<i class="fa-solid fa-eye-slash"></i> Despublicar' : '<i class="fa-solid fa-paper-plane"></i> Publicar escala' });
  pub.addEventListener('click', async () => {
    const publicar = !escala.publicada;
    if (publicar && !(await confirmar({ titulo: 'Publicar escala', mensagem: 'Publicar libera a escala para todos os músicos e será a base dos avisos. Continuar?', okLabel: 'Publicar' }))) return;
    await store.escala.publicar(escala.id, publicar);
    await store.alteracoes.registrar({ escala_id: escala.id, item_id: null, descricao: publicar ? `Escala publicada (v${escala.versao + 1}).` : 'Escala despublicada.', autor: appState.usuario.nome });
    toast(publicar ? 'Escala publicada!' : 'Escala em rascunho.', 'ok');
    appState.recarregar();
  });
...
async function secaoPessoas(alvo, appState) {
  ...
  pessoas.forEach((p) => {
    ...
    row.append(
      ...
      el('div', { style: 'flex:1;min-width:0' }, [
        el('b', {}, [p.nome, p.papel !== 'musico' ? el('span', { class: 'tag', style: 'background:var(--primary);margin-left:6px', text: p.papel }) : null]),
...
function editarPessoa(p, appState) {
  const form = el('div');
  const nome = el('input', { type: 'text', placeholder: 'Nome completo', value: p ? p.nome : '' });
  ...
  form.append(campo('Nome', nome), campo('E-mail', email), campo('Telefone', tel), campo('Papel', papel));
  ...
  abrirModal({
    ...
    acoes: [
      ...
      { label: 'Salvar', classe: 'btn', onClick: async () => {
        ...
        await store.pessoas.salvar({ id: p ? p.id : undefined, nome: nome.value.trim(), email: email.value.trim() || null, telefone: tel.value.trim() || null, papel: papel.value });
